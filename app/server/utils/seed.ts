...
const invoicesData = [
  ...
  {
    customer_id: customersData[7]!.id,
    amount: 54246,
    status: 'pending',
    date: '2023-07-16',
  },
  {
    customer_id: customersData[6]!.id,
    amount: 666,
    status: 'pending',
    date: '2023-06-27',
  },
...
function Seed() {
  (async () => {
    await dbClient.insert(users).values({
      id: 'e80a412a-9987-45a0-b3f8-4292022c7f59',
  ...
  invoicesData.forEach(async (invoice) => {
    await dbClient.insert(invoices).values(invoice);
  });

  customersData.forEach(async (customer) => {
    await dbClient.insert(customers).values(customer);
  });
